...
from collaborative_gym.envs.computer_use.desktop_env.controllers.python import PythonController
from collaborative_gym.envs.computer_use.desktop_env.evaluators.metrics.utils import compare_urls
from collaborative_gym.envs.computer_use.desktop_env.providers.aws.proxy_pool import get_global_proxy_pool, init_proxy_pool, ProxyInfo
    ...
    def _download_setup(self, files: List[Dict[str, str]]):
        ...
        for f in files:
            ...
            if not os.path.exists(cache_path):
                ...
                # Attempt offline fallback before hitting the network.
                offline_sources = []
                env_offline = os.environ.get("OSWORLD_OFFLINE_ASSETS_DIR")
                if env_offline:
                    offline_sources.append(Path(env_offline))
                default_offline = Path(__file__).resolve().parents[2] / "addition" / "cache"
                offline_sources.append(default_offline)

                fallback_used = False
                for offline_root in offline_sources:
                    if not offline_root:
                        continue
                    ...
                    candidate_names = [os.path.basename(path), os.path.basename(cache_path)]
                    for name in candidate_names:
                        search_candidates = []

                        task_id = os.path.basename(self.cache_dir) if self.cache_dir else None
                        if task_id:
                            task_specific_root = offline_root / task_id
                            if task_specific_root.exists():
                                task_candidate = task_specific_root / name
                                if task_candidate.exists():
                                    search_candidates.append(task_candidate)
                                else:
                                    for match in task_specific_root.rglob(name):
                                        if match.is_file():
                                            search_candidates.append(match)
                                            break

                        if not search_candidates:
                            direct_candidate = offline_root / name
                            if direct_candidate.exists():
                                search_candidates.append(direct_candidate)
                            else:
                                # Fall back to recursive search for nested copies
                                for match in offline_root.rglob(name):
                                    if match.is_file():
                                        search_candidates.append(match)
                                        break

                        if search_candidates:
                            source = search_candidates[0]
                            if Path(source).resolve() == Path(cache_path).resolve():
                                fallback_used = True
                                break
                            shutil.copyfile(source, cache_path)
                            logger.info(f"Using offline cached file for {url}: {source}")
                            fallback_used = True
                            break
                    if fallback_used:
                        break
                ...
                if not fallback_used:
                    ...
                    if not downloaded and not os.path.exists(cache_path):
                        raise requests.RequestException(f"Failed to download {url}. No retries left.") from last_error
