...
from collaborative_gym.envs.computer_use.desktop_env.providers.base import Provider
...
class DockerProvider(Provider):
    ...
    def _wait_for_vm_ready(self, timeout: int = 600):
        """Wait for VM to be ready by checking screenshot endpoint. Extended timeout for non-KVM environments."""
    ...
    def start_emulator(self, path_to_vm: str, headless: bool, os_type: str):
        ...
        try:
            with lock:
                # Allocate all required ports (avoiding WebArena conflicts)
                self.vnc_port = self._get_available_port(8006)
                self.server_port = self._get_available_port(6000)  # Changed from 5000 to avoid WebArena routing-car
                self.chromium_port = self._get_available_port(9222)
                self.vlc_port = self._get_available_port(8180)  # Changed from 8080 to avoid WebArena osm-tile-server
